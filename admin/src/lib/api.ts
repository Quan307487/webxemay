...
export const usersApi = { getAll: (p?: any) => api.get('/users', { params: p }), getOne: (id: number) => api.get(`/users/${id}`), updateStatus: (id: number, st: string) => api.put(`/users/${id}/status`, { status: st }) };
